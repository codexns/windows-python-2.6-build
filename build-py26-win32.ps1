...
new-item $buildDir\tcltk -itemtype directory
...
.\ms\do_nasm.bat
nmake.exe -f .\ms\nt.mak
copy-item .\tmp32\buildinf.h .\crypto\buildinf_x86.h
copy-item .\inc32\openssl\opensslconf.h .\crypto\opensslconf_x86.h


cd $buildDir\tcl8.5.17\win
nmake -f makefile.vc COMPILERFLAGS=-DWINVER=0x0500 INSTALLDIR=$buildDir\tcltk clean nmakehlp.exe all
nmake -f makefile.vc INSTALLDIR=$buildDir\tcltk install

cd $buildDir\tk8.5.17\win
nmake -f makefile.vc COMPILERFLAGS=-DWINVER=0x0500 OPTS=noxp INSTALLDIR=$buildDir\tcltk TCLDIR=$buildDir\tcl8.5.17 clean
nmake -f makefile.vc COMPILERFLAGS=-DWINVER=0x0500 OPTS=noxp INSTALLDIR=$buildDir\tcltk TCLDIR=$buildDir\tcl8.5.17 all
nmake -f makefile.vc COMPILERFLAGS=-DWINVER=0x0500 OPTS=noxp INSTALLDIR=$buildDir\tcltk TCLDIR=$buildDir\tcl8.5.17 install
...
msbuild /p:useenv=true .\pcbuild.sln /t:build /p:Configuration=Release /p:Platform=Win32
